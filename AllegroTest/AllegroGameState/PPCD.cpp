...
mask_t *Mask_New(ALLEGRO_BITMAP *bmp)
{
	...
	*/
	//barnhen to comment end
	//barnhen begin
	/*
	If you don’t lock it yourself, every single al_get_pixel()
	command will do their own locks, effectively locking and 
	unlocking the texture hundreds of times.
	
	The reason for this is that part of locking is downloading 
	the texture data so that we can actually get pixel 
	information. If it weren’t locked, the pixel data would 
